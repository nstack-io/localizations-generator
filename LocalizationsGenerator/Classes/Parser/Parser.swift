    ...
    static func parseResponseData(_ data: Data) throws -> ParserOutput {
        ...
        guard let dictionary = object as? [String: AnyObject] else {
            throw NSError(domain: Constants.ErrorDomain.tGenerator.rawValue,
                          ...
                          userInfo:
                [NSLocalizedDescriptionKey : "The data isn't in the correct format. Localizations JSON file should have a dictionary as it's root object."])
        }

        var content: [String: AnyObject]? = dictionary
        content = content?["data"] as? [String : AnyObject] ?? content
        if let t = content?["Localization"] as? [String : AnyObject] {
            content = t
        } else if let t = content?["localizations"] as? [String : AnyObject] {
            content = t
        }
        ...
        guard let langsDictionary = content, let first = langsDictionary.first else {
            throw NSError(domain: Constants.ErrorDomain.tGenerator.rawValue,
                          code: ErrorCode.parserError.rawValue,
                          userInfo: [NSLocalizedDescriptionKey : "Parsed JSON wasn't containing localizations data."])
        }
