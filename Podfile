project 'nstack-translations-generator/nstack-translations-generator.xcodeproj'

# Uncomment the next line to define a global platform for your project
platform :macos, '10.11'

inhibit_all_warnings!

target 'nstack-translations-generator' do
  # Comment the next line if you're not using Swift and don't want to use dynamic frameworks
  use_frameworks!

  # Pods for nstack-translations-generator

end

target 'nstack-translations-generatorBundle' do
  # Comment the next line if you're not using Swift and don't want to use dynamic frameworks
  use_frameworks!

  # Pods for nstack-translations-generatorBundle

end

target 'TranslationsGenerator' do
  # Comment the next line if you're not using Swift and don't want to use dynamic frameworks
  use_frameworks!

  # Pods for TranslationsGenerator

  target 'TranslationsGeneratorTests' do
    inherit! :search_paths
    # Pods for testing
    pod 'Quick'
    pod 'Nimble'
  end

end
